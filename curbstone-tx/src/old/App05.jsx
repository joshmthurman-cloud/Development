...
export default function App() {
    ...
    switch (transType) {
      ...
      case "Return":
        return new Set([
          ...
          "RefId",
          "PaymentType",
          // InvNum optional (not required), keep allowed in case you want to send it
          "InvNum",
          "Token",
          "Expiry",
          "ClerkId",
      ...
      case "Reversal":
        return new Set([
          "TransType",
          // Amount is intentionally excluded for Reversal
          "AuthKey",
          keyReg,
          "RefId",
          "PaymentType",
          // InvNum optional -> keep allowed or remove if you prefer not to send it
          "InvNum",
          "Token",
          "Expiry",
          "ReversalType",
  ...
  const shouldShowField = (field) => {
    ...
    if (["PrintReceipt", "SigCapture", "CardData", "TaxAmount", "LocalTaxFlag", "ClerkId"].includes(field))
      return false;
    return true;
  };
      ...
      <div className="max-w-6xl mx-auto p-6 space-y-6">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            ...
            {shouldShowField("Amount") && (
              <LabeledInput
                ...
                type="number"
                placeholder={
                  transType === "Reversal" ? "Disabled for Reversal" : isMissing("Amount") ? "Required" : "e.g., 1.00"
                }
                missing={isMissing("Amount")}
                disabled={transType === "Reversal"}
              />
            )}
            ...
            {shouldShowField("CustRef") && (
              <LabeledInput label="Customer Number (CustRef)" value={custRef} onChange={setCustRef} placeholder="Optional" />
            )}
        ...
        <section className="bg-white rounded-2xl shadow p-4 space-y-3">
          ...
          <details className="bg-slate-50 rounded-xl p-3 border">
            ...
            <p className="text-sm mt-2">
              We try to auto-capture when the iframe is <em>same-origin</em> and the page contains "{settings.donePhrase}". For
              cross-origin pages (e.g., spinpos.net), browsers block reading the content—use this manual attach if needed.
            </p>
            <textarea
                        ...
                        <Td>
                          ...
                          {l.status === "timeout" && !l.responseCaptured && (
                            <div className="text-xs text-slate-500">
                              Timed out. Paste the response above, then click <em>Attach</em>.
                            </div>
                          )}
        ...
        <section className="bg-white rounded-2xl shadow p-4 space-y-2">
          ...
          <ol className="list-decimal pl-6 space-y-1 text-sm">
            <li>
              Enter your first name to sign in. This becomes <code>ClerkId</code> (hidden from the form).
            </li>
            <li>
              Fill in required fields (inputs show <em>Required</em> when missing for the selected transaction).{" "}
              <strong>Amount</strong> is required for all except <em>Settle</em> and is disabled for <em>Reversal</em>.
            </li>
            <li>
              Click <strong>Execute</strong>. The newest log listens up to 60s; we auto-stop when the iframe loads. If
              same-origin and contains "{settings.donePhrase}", we auto-attach the response.
            </li>
            <li>
              If the iframe is blocked, change <em>Execute behavior</em> to <em>Both (reliable)</em> or <em>Open new tab</em>.
            </li>
            <li>
              Save logs: CSV download or write directly to a chosen <code>Logs/</code> folder.
            </li>
          </ol>
        </section>
      </div>
