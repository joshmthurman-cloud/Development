...
export default function App() {
  ...
  // Required fields map (final rules)
  ...
  const valuesMap = useMemo(
    () => ({
      ...
      RefId: ["Capture", "Reversal"].includes(transType) ? refIdInput : "(auto timestamp)",
      InvNum: invNum,
      Token: token,
      Expiry: expiry,
      CustRef: custRef,
  ...
  const missingRequired = useMemo(() => {
    ...
    return req.filter((k) => {
      ...
      if (k === "RefId") {
        const v = ["Capture", "Reversal"].includes(transType) ? refIdInput : "OK"; // auto for others
        return String(v || "").trim() === "";
      }
      return !valuesMap[k] || String(valuesMap[k]).trim() === "";
    });
  }, [requiredFor, transType, valuesMap, registerId, refIdInput, settings.useRegisterIdSpelling]);
  ...
  const buildXml = (refIdForXml) => {
    ...
    const finalRefId =
      ["Capture", "Reversal"].includes(transType) ? refIdInput.trim() : (refIdForXml || makeRefIdTimestamp());
  ...
  const execute = () => {
    ...
    const finalRefId = ["Capture", "Reversal"].includes(transType) ? refIdInput.trim() : makeRefIdTimestamp();
    ...
    // Optional relay attempts (no-op if not configured)
  ...
  const handleIframeLoad = () => {
    ...
    const tryRead = () => {
      ...
      try {
        ...
        // Cross-origin: can't read. Mark loaded and optionally kick relay.
        setLogs((prev) =>
        ...
        <section className="bg-white rounded-2xl shadow p-4 space-y-4">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            ...
            {shouldShowField("Expiry") && (
              <LabeledInput
                ...
                onChange={setExpiry}
                placeholder={
                  requiredFor[transType]?.includes("Expiry") && isMissing("Expiry") ? "Required (MMYY)" : "MMYY"
                }
                required={requiredFor[transType]?.includes("Expiry")}
          ...
          <div className="bg-slate-50 rounded-xl p-3 text-xs font-mono overflow-auto border">
            ...
            <pre className="whitespace-pre-wrap">
              {buildXml(["Capture", "Reversal"].includes(transType) ? refIdInput : makeRefIdTimestamp())}
            </pre>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <a
              ...
              href={buildUrl(["Capture", "Reversal"].includes(transType) ? (refIdInput || makeRefIdTimestamp()) : makeRefIdTimestamp())}
              target="_blank"
              rel="noreferrer"
            >
              Open in new tab
