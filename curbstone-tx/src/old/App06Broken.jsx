...
export default function App() {
  ...
  const [settings, setSettings] = useState(() =>
    loadJSON(LS_KEYS.settings, {
      ...
      donePhrase: "Done", // try to auto-capture when we can read 'Done' (same-origin only)
      relayCaptureUrl: "", // optional: Cloudflare Worker / proxy endpoint
      relayOnLoaded: true, // when cross-origin page loads, attempt relay capture after 1s
      relayOnExecute: true // also start relay polling immediately after Execute
    })
  );
  ...
  const requiredFor = useMemo(() => ({
    ...
    Sale: ["PaymentType", "TransType", "Amount", "AuthKey", settings.useRegisterIdSpelling, "RefId"],
    Capture: ["PaymentType", "TransType", "Amount", "AuthKey", settings.useRegisterIdSpelling, "RefId", "CustRef"],
    Return: ["TransType", "Amount", "AuthKey", settings.useRegisterIdSpelling, "RefId", "PaymentType", "InvNum", "Token", "Expiry"],
    Reversal: ["TransType", "AuthKey", settings.useRegisterIdSpelling, "RefId", "PaymentType"],
  ...
  const valuesMap = useMemo(() => ({
    ...
    TransType: transType,
    AmountRaw: amount, // use raw input for required check
    Amount: formatAmount(amount),
    AuthKey: authKey,
    [settings.useRegisterIdSpelling]: registerId,
    RefId: (["Capture", "Reversal", "Return"].includes(transType)) ? refIdInput : String(refIdAuto),
    InvNum: invNum,
    Token: token,
    Expiry: expiry,
  }), [paymentType, transType, amount, authKey, registerId, refIdInput, refIdAuto, invNum, token, expiry, settings.useRegisterIdSpelling]);

  const missingRequired = useMemo(() => {
    const req = requiredFor[transType] || [];
    return req.filter((k) => {
      if (k === "Amount") return String(valuesMap.AmountRaw || "").trim() === ""; // required by raw
      if (k === settings.useRegisterIdSpelling) return String(registerId || "").trim() === "";
      if (k === "RefId") {
        const v = (["Capture", "Reversal", "Return"].includes(transType)) ? refIdInput : String(refIdAuto);
  ...
  // Allowed tags per transaction type (to avoid stale tags in XML)
  const allowedTags = useMemo(() => {
    ...
    switch (transType) {
      ...
      case "Sale":
        return new Set([
          "PaymentType", "TransType", "Amount", "AuthKey", keyReg, "RefId", "InvNum", "ClerkId",
          "PrintReceipt", "SigCapture", "CardData", "TaxAmount", "LocalTaxFlag", "CustRef", "DestZipCode"
        ]);
      case "Capture":
        return new Set([
          "PaymentType", "TransType", "Amount", "AuthKey", keyReg, "RefId", "InvNum", "ClerkId",
          "CustRef", "DestZipCode", "TaxAmount", "LocalTaxFlag", "PrintReceipt", "SigCapture"
        ]);
      case "Return":
        return new Set([
          "TransType", "Amount", "AuthKey", keyReg, "RefId", "PaymentType", "InvNum", "Token", "Expiry", "ClerkId"
        ]);
      case "Reversal":
        return new Set([
          "TransType", "AuthKey", keyReg, "RefId", "PaymentType", "ReversalType", "ClerkId"
        ]);
      default:
        return new Set();
    }
  ...
  const buildXml = () => {
    ...
    const pushTag = (name, value) => {
      if (!allowedTags.has(name)) return; // drop tags not allowed for this type
    ...
    const refIdOut = (["Capture", "Reversal", "Return"].includes(transType)) ? refIdInput : String(refIdAuto);
    ...
    // Properties-only but still included when allowed
    pushTag("PrintReceipt", transType === "Capture" ? "No" : printReceipt);
    pushTag("SigCapture", transType === "Capture" ? "No" : sigCapture);
  ...
  const execute = () => {
    ...
    const refOut = (["Capture", "Reversal", "Return"].includes(transType)) ? refIdInput : refIdAuto;
    const logEntry = {
    ...
    // Kick off relay polling immediately if configured
    if (settings.relayOnExecute && settings.relayCaptureUrl) {
      // small delay to ensure currentLogId is set & log exists
  ...
  // Try to auto-capture when iframe loads (same-origin only) and poll for 'Done'
  const handleIframeLoad = () => {
    ...
    const tryRead = () => {
      ...
      try {
        ...
        setLogs((prev) => prev.map((l) => l.id === currentLogId ? { ...l, status: "loaded", awaitUntilMs: undefined } : l));
        clearInterval(pollRef.current);
        pollRef.current = null;
        if (settings.relayOnLoaded && settings.relayCaptureUrl) { setTimeout(() => attemptRelayCapture(currentLogId), 1000); }
        return;
      }

      if (text) {
        // If we can read content, attach it. Optionally check for a 'Done' phrase.
        const hasDone = settings.donePhrase ? text.includes(settings.donePhrase) : true;
        if (hasDone || text.length > 0) {
          setLogs((prev) => prev.map((l) => l.id === currentLogId ? { ...l, status: "captured", responseCaptured: text, awaitUntilMs: undefined } : l));
          clearInterval(pollRef.current);
          pollRef.current = null;
        if (settings.relayOnLoaded && settings.relayCaptureUrl) { setTimeout(() => attemptRelayCapture(currentLogId), 1000); }
        return;
      ...
      if (Date.now() - start > maxMs) {
        // Give up after maxMs
        setLogs((prev) => prev.map((l) => l.id === currentLogId ? { ...l, status: "timeout", awaitUntilMs: undefined } : l));
        clearInterval(pollRef.current);
        pollRef.current = null;
      }
    };
  ...
  // Attempt to capture via a relay/proxy endpoint when cross-origin prevents reading
  const attemptRelayCapture = async (logId, opts = {}) => {
    try {
      ...
      const resp = await fetch(settings.relayCaptureUrl, {
        ...
        body: JSON.stringify({ url: log.requestUrl, refId: log.refId, transType: log.transType, xml: log.requestXml, mode: opts.mode || 'auto' }),
        signal: controller.signal
      });
      const text = await resp.text();
      if (resp.ok && text) {
        setLogs(prev => prev.map(l => l.id === logId ? { ...l, status: 'captured', responseCaptured: text, awaitUntilMs: undefined } : l));
      } else {
        setLogs(prev => prev.map(l => l.id === logId ? { ...l, relayError: `Relay error ${resp.status}`, awaitUntilMs: undefined } : l));
      }
    } catch (e) {
      setLogs(prev => prev.map(l => l.id === logId ? { ...l, relayError: e?.message || String(e), awaitUntilMs: undefined } : l));
    } finally {
  ...
  const attachResponseToMostRecent = () => {
    ...
    setLogs((prev) => prev.map((l, i) => (
      i === 0 ? { ...l, responseCaptured: captureText.trim(), status: "captured", awaitUntilMs: undefined } : l
    )));
    setCaptureText("");
  };
  ...
  const clearLogs = () => {
    ...
    setLogs([]);
    try { localStorage.removeItem(LS_KEYS.logs); } catch {}
  };
  ...
  const saveLogsToFolder = async () => {
    try {
      ...
      while (true) {
        name = `${ymd}${n}.csv`;
        try { await logsDir.getFileHandle(name, { create: false }); n++; }
        catch (err) { if (err?.name === 'NotFoundError') break; else throw err; }
      }
      ...
      <div className="max-w-6xl mx-auto p-6 space-y-6">
        ...
        <details className="bg-white rounded-2xl shadow p-4" open>
          ...
          <div className="grid md:grid-cols-3 gap-4 mt-4">
            ...
            {/* PrintReceipt & SigCapture only here */}
            <label className="flex flex-col gap-1">
            ...
            {/* Relay capture settings */}
            <label className=\"flex flex-col gap-1\">
              <span className=\"text-sm\">Response Relay URL</span>
              <input className=\"px-3 py-2 rounded-xl border\" placeholder=\"https://your-worker.workers.dev\" value={settings.relayCaptureUrl} onChange={(e) => setSettings({ ...settings, relayCaptureUrl: e.target.value })} />
            </label>
            <label className=\"flex items-center gap-2\">
              <input type=\"checkbox\" checked={!!settings.relayOnLoaded} onChange={(e) => setSettings({ ...settings, relayOnLoaded: e.target.checked })} />
              <span className=\"text-sm\">Auto-capture via relay when cross-origin page loads</span>
            </label>
            <label className=\"flex items-center gap-2\">
              <input type=\"checkbox\" checked={!!settings.relayOnExecute} onChange={(e) => setSettings({ ...settings, relayOnExecute: e.target.checked })} />
              <span className=\"text-sm\">Kick off relay immediately at Execute</span>
            </label>
          </div>
        </details>

        {/* Builder - reordered fields with required highlighting */}
        <section className="bg-white rounded-2xl shadow p-4 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-medium">Build Transaction</h2>
            {missingRequired.length > 0 && (
              <div className="text-xs text-red-600" title={missingSummary}>{missingSummary}</div>
            )}
          </div>

          <div className="grid md:grid-cols-3 gap-4">
            {/* Row 1 */}
            <LabeledSelect label="TransType" value={transType} onChange={setTransType} options={TRANS_TYPES} required={true} missing={isMissing('TransType')} />
            {shouldShowField("PaymentType") && <LabeledSelect label="PaymentType" value={paymentType} onChange={setPaymentType} options={PAYMENT_TYPES} required={requiredFor[transType]?.includes('PaymentType')} missing={isMissing('PaymentType')} />}
            {shouldShowField("InvNum") && <LabeledInput label="Invoice Number (InvNum)" value={invNum} onChange={setInvNum} placeholder={requiredFor[transType]?.includes('InvNum') && isMissing('InvNum') ? 'Required' : undefined} missing={isMissing('InvNum')} />}

            {/* Row 2 */}
            {shouldShowField("Amount") && <LabeledInput label="Amount" value={amount} onChange={setAmount} type="number" placeholder={transType === 'Reversal' ? 'Disabled for Reversal' : (isMissing('Amount') ? 'Required' : 'e.g., 1.00')} missing={isMissing('Amount')} disabled={transType === 'Reversal'} />}
            {shouldShowField("Address") && <LabeledInput label="Address" value={address} onChange={setAddress} placeholder="Optional" />}
            {shouldShowField("Zip") && <LabeledInput label="Zip" value={zip} onChange={setZip} placeholder="Optional" />}

            {/* Row 3 */}
            {shouldShowField("CustRef") && <LabeledInput label="Customer Number (CustRef)" value={custRef} onChange={setCustRef} placeholder="Optional" />}
            {shouldShowField("DestZipCode") && <LabeledInput label="Destination Zip (DestZipCode)" value={destZipCode} onChange={setDestZipCode} placeholder="Optional" />}
            {shouldShowField("RefId") && (
              (["Capture", "Reversal", "Return"].includes(transType)) ? (
                <LabeledInput label="MFUKEY (RefId)" value={refIdInput} onChange={setRefIdInput} placeholder={isMissing('RefId') ? 'Required' : undefined} missing={isMissing('RefId')} />
              ) : (
            ...
            {/* Row 4 */}
            {shouldShowField("AuthKey") && <LabeledInput label="AuthKey" value={authKey} onChange={setAuthKey} placeholder={isMissing('AuthKey') ? 'Required' : undefined} missing={isMissing('AuthKey')} />}
            {shouldShowField(settings.useRegisterIdSpelling) && <LabeledInput label={settings.useRegisterIdSpelling} value={registerId} onChange={setRegisterId} placeholder={isMissing(settings.useRegisterIdSpelling) ? 'Required' : undefined} missing={isMissing(settings.useRegisterIdSpelling)} />}

            {/* Optional, appear when applicable */}
            {shouldShowField("AuthCode") && <LabeledInput label="AuthCode" value={authCode} onChange={setAuthCode} placeholder="Optional" />}
            {shouldShowField("Token") && <LabeledInput label="Token" value={token} onChange={setToken} placeholder={requiredFor[transType]?.includes('Token') && isMissing('Token') ? 'Required' : 'Optional'} missing={isMissing('Token')} />}
            {shouldShowField("Expiry") && <LabeledInput label="Expiry" value={expiry} onChange={setExpiry} placeholder={requiredFor[transType]?.includes('Expiry') && isMissing('Expiry') ? 'Required (MMYY)' : 'MMYY'} missing={isMissing('Expiry')} />}
          </div>
          ...
          <div className="flex items-center gap-3">
            <button onClick={execute} disabled={!canExecute} title={missingSummary} className={`px-4 py-2 rounded-2xl text-white hover:opacity-95 ${canExecute ? 'bg-emerald-600' : 'bg-slate-400 cursor-not-allowed'}`}>
              Execute ? Load below
            </button>
            <a
        ...
        <section className="bg-white rounded-2xl shadow p-4 space-y-3">
          ...
          <details className="bg-slate-50 rounded-xl p-3 border">
            ...
            <p className="text-sm mt-2">
              We try to auto-capture when the iframe is <em>same-origin</em> and the page contains "{settings.donePhrase}". For cross-origin pages (e.g., spinpos.net), browsers block reading the content—use this manual attach if needed.
            </p>
            <textarea
        ...
        <section className="bg-white rounded-2xl shadow p-4 space-y-2">
          ...
          <ol className="list-decimal pl-6 space-y-1 text-sm">
            ...
            <li>Fill in required fields (inputs show <em>Required</em> when missing for the selected transaction). <strong>Amount</strong> is required for all except <em>Settle</em>.</li>
            <li>Click <strong>Execute</strong>. The newest log listens up to 60s; we auto-stop when the iframe loads. If same-origin and contains "{settings.donePhrase}", we auto-attach the response.</li>
...
// ---- UI helpers ----
function LabeledInput({ label, value, onChange, type = "text", placeholder, required = false, missing = false, disabled = false }) {
  return (
    <label className="flex flex-col gap-1">
      <span className="text-sm flex items-center gap-2">
        {label}
        {required && <span className={`text-[10px] px-2 py-0.5 rounded-full ${missing ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-500'}`}>Required</span>}
      </span>
      <input
...
function LabeledSelect({ label, value, onChange, options, required = false, missing = false }) {
  return (
    <label className="flex flex-col gap-1">
      ...
      <select className={`px-3 py-2 rounded-xl border ${missing ? 'border-red-400 ring-1 ring-red-200' : ''}`} value={value} onChange={(e) => onChange(e.target.value)}>
        {options.map((o) => (
