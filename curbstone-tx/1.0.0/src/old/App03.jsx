...
export default function App() {
  ...
  // Track most recent log for auto-capture
  ...
  const missingRequired = useMemo(() => {
    ...
    return req.filter((k) => !valuesMap[k] || String(valuesMap[k]).trim() === "");
  }, [requiredFor, transType, valuesMap]);
  ...
  const buildXml = () => {
    ...
    pushTag("RefId", (["Capture", "Reversal", "Return"].includes(transType)) ? refIdInput : String(refIdAuto));
    pushTag("InvNum", invNum);
    pushTag("ClerkId", clerkId);
    // Controlled from Properties & Defaults only
  ...
  const execute = () => {
    ...
    const logEntry = {
      id: `${Date.now()}_${(["Capture", "Reversal", "Return"].includes(transType) ? refIdInput : refIdAuto)}`,
      timestamp: new Date().toISOString(),
      user: clerkId,
      transType,
      amount: formatAmount(amount),
      refId: (["Capture", "Reversal", "Return"].includes(transType) ? refIdInput : refIdAuto),
      requestXml: buildXml(),
      requestUrl: url,
  ...
  // Try to auto-stop listening & capture when iframe loads
  const handleIframeLoad = () => {
    ...
    try {
      ...
      // Cross-origin; we can't read contents. We'll still mark as loaded.
    }
    setLogs((prev) => prev.map((l) => {
      if (l.id !== currentLogId) return l;
      return {
        ...l,
        status: captured ? "captured" : "loaded",
        awaitUntilMs: undefined,
        responseCaptured: captured || l.responseCaptured,
      };
    }));
  };
      ...
      <div className="max-w-6xl mx-auto p-6 space-y-6">
        ...
        <section className="bg-white rounded-2xl shadow p-4 space-y-3">
          ...
          <details className="bg-slate-50 rounded-xl p-3 border">
            ...
            <p className="text-sm mt-2">
              The newest log will listen for up to <strong>60s</strong>. When the iframe finishes loading, we'll stop the timer and mark it as <em>loaded</em> (and auto-capture the text if the page is same-origin). For cross-origin pages, paste below and click <em>Attach</em>.
            </p>
            <textarea
        ...
        <section className="bg-white rounded-2xl shadow p-4 space-y-2">
          ...
          <ol className="list-decimal pl-6 space-y-1 text-sm">
            ...
            <li>Fill in required fields (inputs show <em>Required</em> when missing for the selected transaction).</li>
            <li>Click <strong>Execute</strong>. The newest log listens up to 60s; we auto-stop when the iframe finishes loading.</li>
  ...
  return (
    <label className="flex flex-col gap-1">
      <span className="text-sm flex items-center gap-2">
        {label}
        {required && <span className={`text-[10px] px-2 py-0.5 rounded-full ${missing ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-500'}`}>{missing ? 'Required' : 'Required'}</span>}
      </span>
      <input
