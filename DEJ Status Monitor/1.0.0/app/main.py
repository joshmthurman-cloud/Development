...
from app.models import Terminal, StatusCheck, User, UserMerchant, UserRole
...
async def reload_tpns(
    ...
    try:
        ...
        terminals_before = db.query(Terminal).count()
        ...
        terminals_after = db.query(Terminal).count()
        
        # Build detailed message
        message_parts = [
            f"Reloaded {results['total_tpns']} TPNs from STEAM",
            f"File: {tpns_before} → {tpns_after} terminals ({tpns_after - tpns_before:+d})",
            f"Database: {terminals_before} → {terminals_after} terminals ({terminals_after - terminals_before:+d})",
        ...
        return {
            ...
            "details": {
                ...
                "tpns_file_before": tpns_before,
                "tpns_file_after": tpns_after,
                "terminals_db_before": terminals_before,
                "terminals_db_after": terminals_after
            }
        }
...
async def get_merchants(db: Session = Depends(get_db)):
    """Get list of all merchants with company names"""
    from app.services.merchant_loader import load_merchant_mapping
    
    terminals = db.query(Terminal.tpn).all()
    merchant_codes = set()
...
async def approve_user(
    ...
    <body>
        ...
        <p><a href="http://localhost:8000/login">Click here to log in</a></p>
        <p>If you have any questions, please contact your administrator.</p>
    </body>
    </html>
    """
...
async def index(
    ...
    # Count terminals in file
    from app.services.tpn_loader import count_tpns_in_file
    tpns_file_count = count_tpns_in_file(TPN_FILE_PATH)
    
    # Count terminals in database
    total_terminals_in_db = db.query(Terminal).count()
    ...
    return templates.TemplateResponse("index.html", {
        ...
        "tpns_file_count": tpns_file_count,
        "total_terminals_in_db": total_terminals_in_db,
...
async def get_terminal_counts(db: Session = Depends(get_db)):
    """Get terminal counts for display in header"""
    from app.services.tpn_loader import count_tpns_in_file
    tpns_file_count = count_tpns_in_file(TPN_FILE_PATH)
    total_terminals_in_db = db.query(Terminal).count()
    return {
